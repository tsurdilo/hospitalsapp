...
public class HospitalTemplateOperationsManualTest {
    ...
    public void setUp() throws Exception {
        hospitalTemplate.deleteAll();
    }

    @Test
    public void testSaveHospital() {
        Hospital hospital = hospitalTemplate
                .save(Mono.just(new Hospital("northside", "Northside Hospital", "some address", 30040, null))).block();
        assertNotNull(hospital.getId());
    }

    @Test
    public void testfindHospitalById() {
        Mono<Hospital> hospitalMono = hospitalTemplate
                .save(Mono.just(new Hospital("northside", "Northside Hospital", "some address", 30040, null)));
        Mono<Hospital> hospitalMonoResult = hospitalTemplate.findById(hospitalMono.block().getId());
    ...
    public void testFindAllHospitals() {
        Hospital hospital1 = hospitalTemplate
                .save(Mono.just(new Hospital("northside", "Northside Hospital", "some address", 30040, null))).block();
        Hospital hospital2 = hospitalTemplate
                .save(Mono.just(new Hospital("northside", "Northside Hospital", "some address", 30040, null))).block();
        Flux<Hospital> hospitalFlux = hospitalTemplate.findAll();
    ...
    public void testFindAllHospitalsWithPatients() {
        ...
        Mono<Hospital> hospitalMono = hospitalTemplate.save(Mono.just(hospital));
